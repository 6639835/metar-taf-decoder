...
from ..utils.patterns import COMPILED_PATTERNS, RVR_PATTERN, RUNWAY_STATE_PATTERN
from ..utils.constants import TREND_TYPES, MILITARY_COLOR_CODES, RUNWAY_DEPOSIT_TYPES, RUNWAY_EXTENT, RUNWAY_BRAKING
...
class MetarDecoder:
    ...
    def _extract_windshear(self, parts: List[str]) -> List[Dict]:
        ...
        while i < len(parts):
            if parts[i] == 'WS':
                ...
                while i < len(parts) and parts[i] in ['RWY', 'ALL', 'TKOF', 'LDG'] or \
                      (i < len(parts) and re.match(r'\d{2}[LCR]?', parts[i])):
        ...
        while i < len(parts):
            if parts[i] in TREND_TYPES:
                ...
                while i < len(parts) and parts[i] not in TREND_TYPES and not parts[i].startswith('RMK'):
                    ...
                    elif re.match(r'^[-+]?VC?(MI|PR|BC|DR|BL|SH|TS|FZ)?(DZ|RA|SN|SG|IC|PL|GR|GS|UP|BR|FG|FU|VA|DU|SA|HZ|PY|PO|SQ|FC|SS|DS)+', element):
    ...
    def _extract_remarks(self, metar: str) -> tuple:
        ...
        if match:
            ...
            VALID_PAST_WEATHER_CODES = {'RA', 'SN', 'DZ', 'TS', 'PL', 'GR', 'GS', 'UP', 'FG', 'BR', 'FU', 'VA', 'DU', 'SA', 'HZ', 'SQ', 'FC', 'SS', 'DS'}
            ...
            virga_match = re.search(r'VIRGA\s*(DSNT|VC)?\s*((?:(?:NE|NW|SE|SW|N|E|S|W)(?:-(?:NE|NW|SE|SW|N|E|S|W))?)(?:\s+AND\s+(?:NE|NW|SE|SW|N|E|S|W)(?:-(?:NE|NW|SE|SW|N|E|S|W))?)*)?', remarks)
            if virga_match:
            ...
            acsl_match = re.search(r'ACSL\s*(DSNT|VC|OHD)?\s*([NSEW]+(?:-[NSEW]+)?)?\s*(?:MOV\s+([NSEW]+(?:-[NSEW]+)?))?', remarks)
            if acsl_match:
            ...
            if runway_state_rmk_match:
                ...
                decoded['Runway State (Remarks)'] = f"{runway_desc}: {deposit_desc}, {extent_desc} coverage, depth {depth_desc}, braking {braking_desc}"
