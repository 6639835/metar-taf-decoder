...
class VisibilityParser:
    ...
    def extract_visibility(parts: List[str]) -> Optional[Dict]:
        ...
        for i, part in enumerate(parts):
            ...
            if dir_match:
                ...
                parts.pop(i)
                return {"value": vis_value, "unit": "M", "is_cavok": False, "direction": direction}
            ...
            if part.isdigit() and i + 1 < len(parts):
                ...
                if frac_match:
                    ...
                    return {"value": whole + (numerator / denominator), "unit": "SM", "is_cavok": False}
            ...
            if sm_match:
                ...
                parts.pop(i)
                result = {"value": numerator / denominator, "unit": "SM", "is_cavok": False}
            ...
            if part.endswith("NDV"):
                ...
                if numeric_part.isdigit() and len(numeric_part) == 4:
                    ...
                    parts.pop(i)
                    return {"value": vis_value, "unit": "M", "is_cavok": False, "ndv": True}
    ...
    def parse_visibility_string(vis_str: str) -> Optional[Dict]:
        ...
        if dir_match:
            return {"value": int(dir_match.group(1)), "unit": "M", "is_cavok": False, "direction": dir_match.group(2)}

        # Check for standard 4-digit format
        if len(vis_str) == 4 and vis_str.isdigit():
            return {"value": int(vis_str), "unit": "M", "is_cavok": False}

        # Check for NDV format
        if vis_str.endswith("NDV"):
            numeric_part = vis_str[:-3]
            if numeric_part.isdigit() and len(numeric_part) == 4:
                return {"value": int(numeric_part), "unit": "M", "is_cavok": False, "ndv": True}
